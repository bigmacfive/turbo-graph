...
#[derive(Clone, Copy)]
pub(crate) struct SearchMask<'a> {
    pub(crate) words: &'a [u64],
    pub(crate) count: usize,
    pub(crate) active_blocks: &'a [usize],
}

struct BlockIter<'a> {
    active_blocks: Option<&'a [usize]>,
    pos: usize,
    end: usize,
}
...
impl Iterator for BlockIter<'_> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if let Some(blocks) = self.active_blocks {
            let block = *blocks.get(self.pos)?;
            self.pos += 1;
            Some(block)
...
#[inline(always)]
fn block_iter(n_blocks: usize, mask: Option<SearchMask<'_>>) -> BlockIter<'_> {
    BlockIter {
        active_blocks: mask.map(|m| m.active_blocks),
        pos: 0,
        end: n_blocks,
    }
}
...
/// early-exit path. Incremented whenever a precomputed mask block-list
/// excludes a block, or by the x86 AVX-512 pair guard when it skips blocks
/// because no allowed slots fall within them.
///
...
#[inline(always)]
fn record_preselected_block_skips(n_blocks: usize, mask: Option<SearchMask<'_>>) {
    if let Some(mask) = mask {
        let skipped = n_blocks.saturating_sub(mask.active_blocks.len());
        if skipped > 0 {
            BLOCKS_SKIPPED_BY_MASK.fetch_add(skipped as u64, Ordering::Relaxed);
        }
    }
}
...
unsafe fn search_multi_query_avx2(
    ...
    for b in block_iter(n_blocks, mask) {
        ...
        for qi in 0..nq {
            ...
            if *sz < k {
                for lane in 0..(end - base_vec) {
                    if let Some(m) = mask {
                        if !mask_allows(m.words, base_vec + lane) {
                            continue;
                        }
                    }
...
// extracts both halves into `__m256i` locals and runs the same combine,
// convert, fmadd, norm-mul, and heap-update sequence for each block.
//
...
/// runtime (e.g. running a turbo-graph binary built without the cargo
...
pub(crate) fn search(
    ...
    record_preselected_block_skips(n_blocks, mask);
        ...
        let results: Vec<Vec<(Vec<f32>, Vec<i64>)>> = (0..nq)
            ...
            .map(|qi_start| {
                ...
                if batch_size == QBS {
                    ...
                    for block_idx in block_iter(n_blocks, mask) {
                ...
                (0..batch_size)
                    .map(|qi_off| {
                        ...
                        let mut consider = |i: usize, s: f32| {
                            if heap_sz < k {
                                heap_s[heap_sz] = s;
                        ...
                        if let Some(m) = mask {
                            for &block in m.active_blocks {
                                let start = block * BLOCK;
                                let end = (start + BLOCK).min(n_vectors);
                                for i in start..end {
                                    if !mask_allows(m.words, i) {
                                        continue;
                                    }
                                    consider(i, row[i]);
                                }
