...
pub fn repack(packed_codes: &[u8], n_vectors: usize, bits: usize, dim: usize) -> (Vec<u8>, usize) {
...
pub fn repack_3bit(packed_codes: &[u8], n_vectors: usize, dim: usize) -> (Vec<u8>, Vec<u8>, usize) {
    let bytes_per_plane = dim / 8;
    ...
    for block_idx in 0..n_blocks {
        ...
        for g in 0..sub_byte_groups {
            ...
            for lane in 0..BLOCK {
                ...
                for c in 0..4usize {
                    ...
                    let mut code = 0u8;
                    for p in 0..2usize {
                        let plane_byte = packed_codes
                            [vec_idx * bytes_per_row + p * bytes_per_plane + byte_in_plane];
                        if plane_byte & mask != 0 {
                            code |= 1 << p;
                        }
                    }
        ...
        for g in 0..plane2_byte_groups {
            ...
            for lane in 0..BLOCK {
                ...
                plane2_blocked[out_offset + lane] =
