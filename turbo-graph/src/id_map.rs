...
use crate::io;
use crate::{AddError, ConstructError, SlotMask, TurboQuantIndex};
...
impl IdMapIndex {
    ...
    pub fn search_with_allowlist(
        ...
        let mask_buf: Option<SlotMask> = allowlist.map(|ids| {
            assert!(!ids.is_empty(), "allowlist is empty");
            let mut mask = SlotMask::new(self.inner.len());
            for &id in ids {
        ...
        let res = match mask_buf.as_ref() {
            Some(mask) => self.inner.search_with_slot_mask(queries, k, mask),
            None => self.inner.search(queries, k),
        };
