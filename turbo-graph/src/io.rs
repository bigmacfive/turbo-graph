//! Read/write turbo-graph index files.
//!
...
//! ## Format versioning
//!
//! Both formats are at version 3 as of turbo-graph 0.6.x (TQ+ per-coord
//! calibration). Version 2 (turbo-graph 0.4.4 .. 0.6.0) is loaded transparently
...
//! up the new calibration. Version 1 (turbo-graph ≤ 0.4.3) is incompatible
...
//! lets us detect either a current file or "looks like a v1 turbo-graph
//! file" cleanly.
...
const REBUILD_HINT: &str =
    "Rebuild this index from the source vectors using turbo-graph 0.4.4 or later \
...
pub fn load(path: impl AsRef<Path>) -> io::Result<CoreLoad> {
    ...
    if &magic != TV_MAGIC {
        ...
        // message; otherwise treat it as a non-turbo-graph file.
        if (2..=4).contains(&magic[0]) {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!(
                    "this .tv file was written by turbo-graph ≤ 0.4.3 (format \
                     version 1). It is incompatible with turbo-graph 0.4.4+ \
        ...
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            "not a turbo-graph .tv file: wrong magic",
        ));
    }
...
pub fn load_id_map(
    ...
    if version[0] == 1 {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!(
                "this .tvim file was written by turbo-graph ≤ 0.4.3 (format \
                 version 1). It is incompatible with turbo-graph 0.4.4+ \
                 because the per-vector scalar's meaning changed. {}",
                REBUILD_HINT,
            ),
