...
pub use graph_memory::{
    GraphBatchPlannedSearchReport, GraphBatchRerankedSearchReport,
    GraphBatchTimedRerankedSearchReport, GraphCandidateBatchHybridSearchReport,
    GraphCandidateBatchRerankedSearchReport, GraphCandidateExplainedSearchReport,
    GraphCandidateHybridExplainedSearchReport, GraphCandidateHybridSearchDebugSnapshot,
    GraphCandidateHybridSearchReport, GraphCandidatePlannedSearchReport,
    GraphCandidateRerankedSearchReport, GraphCandidateScoreNormalization,
    GraphCandidateSearchDebugSnapshot, GraphCandidateSearchDebugSummary, GraphCandidateSearchPlan,
    GraphCandidateTimedHybridSearchReport, GraphCandidateTimedRerankedSearchReport,
    GraphExplainedSearchReport, GraphHybridHit, GraphHybridRerankConfig, GraphHybridSearchDebugHit,
    GraphHybridSearchDebugNode, GraphMemoryCacheBudget, GraphMemoryCacheStats, GraphMemoryError,
    GraphMemoryIndex, GraphPlannedSearchReport, GraphPreparedPolicyView, GraphPreparedView,
    GraphRerankConfig, GraphRerankedHit, GraphRerankedSearchReport, GraphSearchDebugEdge,
    GraphSearchDebugHit, GraphSearchDebugNode, GraphSearchDebugSnapshot, GraphSearchDebugSummary,
    GraphSearchPlan, GraphSearchPreset, GraphSearchReport, GraphSearchTelemetry,
    GraphSearchTraceReport, GraphSearchTuning, GraphTimedRerankedSearchReport, GraphViewEdge,
    GraphViewNode, GraphViewPolicy, GraphViewStats, GraphViewTrace, MemoryEdge, MemoryHit,
    MemoryRecord,
};
pub use id_map::IdMapIndex;
pub use mask::{AllowedSlots, SlotMask};
...
impl TurboQuantIndex {
    ...
    pub fn search_with_mask(
        ...
        let packed_mask = mask.map(|m| {
            ...
            let mut slot_mask = SlotMask::new(self.n_vectors);
        ...
        match packed_mask.as_ref() {
            Some(slot_mask) => self.search_with_slot_mask(queries, k, slot_mask),
            None => self.search_with_packed_slot_mask(queries, k, None),
        }
    }

    /// Run a top-`k` search restricted by a cached packed slot mask.
    ///
    /// This is the low-allocation filtering path for callers that reuse the
    /// same candidate set across many searches (for example graph views,
    /// metadata facets, ACLs, or time windows). It avoids the `&[bool]` to
    /// packed-bit conversion performed by [`Self::search_with_mask`].
    ///
    /// # Panics
    ///
    /// Panics if `mask.len() != self.len()`, or on the same query-shape and
    /// invalid-value conditions as [`Self::search`].
    pub fn search_with_slot_mask(
        ...
        assert_eq!(
            mask.len(),
            self.n_vectors,
            "SlotMask length {} does not match index size {}",
            mask.len(),
            self.n_vectors,
        );
        if mask.is_all() {
            self.search_with_packed_slot_mask(queries, k, None)
        } else {
            let (words, count, active_blocks) = mask.search_parts();
            self.search_with_packed_slot_mask(queries, k, Some((words, count, active_blocks)))
        }
    }

    fn search_with_packed_slot_mask(
        &self,
        queries: &[f32],
        k: usize,
        packed_mask: Option<(&[u64], usize, &[usize])>,
    ) -> SearchResults {
        ...
        let (search_mask, n_allowed) = match packed_mask {
            Some((words, count, active_blocks)) => {
                let expected_words = (self.n_vectors + 63) / 64;
                assert_eq!(
                    words.len(),
                    expected_words,
                    "packed mask word length {} does not match index size {} (expected {})",
                    words.len(),
                    self.n_vectors,
                    expected_words,
                );
                for &block in active_blocks {
                    assert!(
                        block < blocked.n_blocks,
                        "active mask block {block} out of range for {} blocks",
                        blocked.n_blocks,
                    );
                }
                (
                    Some(search::SearchMask {
                        words,
                        count,
                        active_blocks,
                    }),
