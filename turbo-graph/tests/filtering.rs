...
use turbo_graph::{IdMapIndex, SlotMask, TurboQuantIndex};
...
#[test]
fn packed_slot_mask_matches_bool_mask() {
    let dim = 128;
    let n = 512;
    let idx = build_index(n, dim, 0xF11D_2001);
    let queries = gaussian_normalized(3, dim, 0xF11D_2002);

    let allowed_slots: Vec<usize> = (0..n)
        .filter(|slot| slot % 7 == 0 || slot % 31 == 0)
        .collect();
    ...
    let packed = idx.search_with_slot_mask(&queries, 12, &slot_mask);
    let boolean = idx.search_with_mask(&queries, 12, Some(&bool_mask));

    assert_eq!(packed.nq, boolean.nq);
    assert_eq!(packed.k, boolean.k);
    assert_eq!(packed.scores, boolean.scores);
    assert_eq!(packed.indices, boolean.indices);
}
...
fn all_slot_mask_uses_unmasked_fast_path() {
    let dim = 64;
    let n = 1024;
    let idx = build_index(n, dim, 0xF11D_2003);
    let queries = gaussian_normalized(2, dim, 0xF11D_2004);
    let mask = SlotMask::all(n);

    let masked = idx.search_with_slot_mask(&queries, 16, &mask);
    let unmasked = idx.search(&queries, 16);

    assert_eq!(masked.nq, unmasked.nq);
    assert_eq!(masked.k, unmasked.k);
    assert_eq!(masked.scores, unmasked.scores);
    assert_eq!(masked.indices, unmasked.indices);
}
...
#[test]
fn pre_filtering_returns_view_top_k_when_post_filter_underfetches() {
    let dim = 64;
    let n = 12;
    let mut data = vec![0.0f32; n * dim];
    for row in 0..10 {
    ...
    bool_mask[10] = true;
    bool_mask[11] = true;
    let slot_mask = SlotMask::from_slots(n, [10, 11]);

    let global = idx.search(&query, 4);
    let post_filtered: Vec<i64> = global
        .indices_for_query(0)
        .iter()
        .copied()
        .filter(|&slot| bool_mask[slot as usize])
        .collect();
    assert!(
        post_filtered.is_empty(),
        "under-fetched global post-filter unexpectedly found allowed slots: {post_filtered:?}"
    );

    let masked = idx.search_with_mask(&query, 2, Some(&bool_mask));
    let packed = idx.search_with_slot_mask(&query, 2, &slot_mask);
    assert_eq!(masked.indices_for_query(0), &[10, 11]);
    assert_eq!(packed.indices_for_query(0), masked.indices_for_query(0));
    assert_eq!(packed.scores_for_query(0), masked.scores_for_query(0));
}
