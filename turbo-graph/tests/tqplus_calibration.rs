...
use turbo_graph::{io, TurboQuantIndex};
...
fn empty_first_add_does_not_freeze_identity_calibration() {
    ...
    let tmp = std::env::temp_dir().join(format!(
        "turbo-graph_empty_add_freeze_{}.tv",
        std::process::id()
    ));
...
fn v2_loaded_index_populates_identity_calibration() {
    ...
    let path = std::env::temp_dir().join(format!(
        "turbo-graph_v2_load_then_add_{}.tv",
        std::process::id()
    ));
    let bit_width = 4u8;
    let dim = 128u32;
    ...
    f.write_all(&vec![
        0u8;
        ((dim / 8) * (bit_width as u32) * n_vectors) as usize
    ])
    .unwrap();
    ...
    let tmp = std::env::temp_dir().join(format!(
        "turbo-graph_v2_load_then_add_out_{}.tv",
        std::process::id()
    ));
    idx.write(&tmp).unwrap();
    let (_, _, _, _, _, shift, scale_tq) = io::load(&tmp).unwrap();
