...
fn add_with_ids_rejects_duplicate_id() {
    ...
    assert_eq!(err, turbo_graph::AddError::IdAlreadyPresent(2));
}
...
fn add_with_ids_rejects_length_mismatch() {
    ...
    assert_eq!(
        err,
        turbo_graph::AddError::IdsCountMismatch {
            expected: 5,
            got: 3,
        },
    );
...
fn write_and_load_round_trips() {
    ...
    let tmp = std::env::temp_dir().join(format!("turbo-graph_idmap_{}.tvim", std::process::id()));
...
fn load_rejects_wrong_magic() {
    let tmp = std::env::temp_dir().join(format!(
        "turbo-graph_idmap_badmagic_{}.tvim",
        std::process::id()
    ));
...
fn add_with_ids_2d_rejects_non_multiple_buffer() {
    ...
    assert!(
        matches!(
            err,
            turbo_graph::AddError::VectorBufferNotMultipleOfDim { .. }
        ),
