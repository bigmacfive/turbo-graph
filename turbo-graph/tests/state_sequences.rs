...
fn second_add_after_search_lets_new_vectors_be_found() {
    ...
    assert_eq!(
        res.indices[0] as usize, 5,
        "new vector not findable after second add+search"
    );
}
...
fn swap_remove_after_load_produces_correct_search() {
    ...
    assert_eq!(
        r4.indices[0] as usize, 1,
        "moved vector should now be at slot 1"
    );
}
...
fn swap_remove_then_round_trip_matches_in_memory_search() {
    ...
    let tmp = std::env::temp_dir().join(format!(
        "turbo-graph_seq_swap_roundtrip_{}.tv",
        std::process::id()
    ));
    idx.write(&tmp).unwrap();
...
fn id_map_re_added_id_returns_new_vector_not_old() {
    ...
    assert_eq!(
        ids[0], 42,
        "self-query of re-added vector should return its id"
    );
}
...
fn prepare_then_add_invalidates_blocked_cache() {
    ...
    assert_eq!(
        ...
        "new vector not findable after prepare+add"
    );
}
...
fn add_after_load_extends_index() {
    ...
    let tmp = std::env::temp_dir().join(format!(
        "turbo-graph_seq_add_after_load_{}.tv",
        std::process::id()
    ));
    idx.write(&tmp).unwrap();
    let mut loaded = TurboQuantIndex::load(&tmp).unwrap();
    ...
    assert_eq!(
        res.indices[0] as usize, 3,
        "new vector should be at slot 3 after load+add"
    );
}
...
fn prepare_then_swap_remove_invalidates_cache() {
    ...
    assert_ne!(
        res.indices[0] as usize, 1,
        "deleted vector should not be retrievable after prepare+swap_remove"
    );
}
