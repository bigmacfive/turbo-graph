...
use turbo_graph::io::{load, load_id_map, write, write_id_map};
...
fn temp_path(name: &str) -> PathBuf {
    ...
    let nonce = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    p.push(format!("turbo-graph-{}-{}", nonce, name));
    p
}
...
fn tv_v1_file_is_rejected_with_upgrade_hint() {
    // Hand-construct a turbo-graph ≤ 0.4.3 `.tv` file: bare header
    ...
    assert!(
        msg.contains("turbo-graph ≤ 0.4.3") && msg.contains("Rebuild"),
...
fn tvim_v1_file_is_rejected_with_upgrade_hint() {
    // Hand-construct a turbo-graph ≤ 0.4.3 `.tvim` file: TVIM magic, version
...
fn tv_garbage_file_rejected_without_upgrade_hint() {
    ...
    assert!(
        !msg.contains("turbo_graph ≤ 0.4.3") && !msg.contains("turbo-graph ≤ 0.4.3"),
