...
use std::fs;
use turbo_graph::{IdMapIndex, TurboQuantIndex};
...
fn add_2d_rejects_dim_change() {
    ...
    assert_eq!(
        err,
        turbo_graph::AddError::DimMismatch {
            existing: DIM,
            got: DIM * 2,
        },
    );
...
fn add_2d_rejects_non_multiple_of_8_dim_on_lazy_index() {
    ...
    assert_eq!(err, turbo_graph::AddError::DimNotMultipleOf8(7));
...
fn write_load_round_trip_lazy_uncommitted() {
    let tmp = std::env::temp_dir().join("turbo-graph_lazy_uncommitted.tv");
    {
        let idx = TurboQuantIndex::new_lazy(4).unwrap();
...
fn write_load_round_trip_eager_index_still_works() {
    ...
    let tmp = std::env::temp_dir().join("turbo-graph_lazy_eager.tv");
    {
        let mut idx = TurboQuantIndex::new(DIM, 4).unwrap();
...
fn write_load_round_trip_lazy_after_committed_add() {
    let tmp = std::env::temp_dir().join("turbo-graph_lazy_committed.tv");
    {
...
fn id_map_write_load_round_trip_lazy_uncommitted() {
    let tmp = std::env::temp_dir().join("turbo-graph_idmap_lazy_uncommitted.tvim");
    {
...
fn id_map_write_load_round_trip_lazy_after_committed_add() {
    let tmp = std::env::temp_dir().join("turbo-graph_idmap_lazy_committed.tvim");
...
fn new_rejects_bad_bit_width() {
    for bw in [0usize, 1, 5, 8, 100] {
        ...
        assert_eq!(err, turbo_graph::ConstructError::BitWidthOutOfRange(bw));
    }
}
...
fn new_rejects_bad_dim() {
    for dim in [0usize, 1, 4, 7, 9, 15] {
        ...
        assert_eq!(
            err,
            turbo_graph::ConstructError::DimNotPositiveMultipleOf8(dim)
        );
    }
}
...
fn id_map_new_rejects_bad_bit_width() {
    ...
    assert_eq!(err, turbo_graph::ConstructError::BitWidthOutOfRange(5));
}
...
fn id_map_new_rejects_bad_dim() {
    ...
    assert_eq!(
        err,
        turbo_graph::ConstructError::DimNotPositiveMultipleOf8(0)
    );
}
