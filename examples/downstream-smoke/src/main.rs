//! Smoke test for the downstream `cargo add turbo-graph` experience.
//!
...
//! runs, the link-directive propagation from turbo-graph's build.rs is
...
fn main() {
    ...
    let tmp = std::env::temp_dir().join("turbo-graph_downstream_smoke.tv");
