"""LangChain VectorStore backed by turbo_graph's quantized index.

Install with: ``pip install turbo-graph[langchain]``.
...
try:
    ...
    raise ImportError(
        "langchain-core is required to use turbo_graph.langchain. "
        "Install with: pip install turbo-graph[langchain]"
    ) from exc
...
class TurboQuantVectorStore(VectorStore):
    ...
    def _select_relevance_score_fn(self) -> Callable[[float], float]:
        # turbo_graph returns the raw inner product of unit-normalized vectors —
    ...
    # pairwise diversity scores. turbo_graph discards full vectors after
    ...
    def dump(self, folder_path: str | Path) -> None:
        ...
        ``folder_path`` is a directory; turbo_graph writes ``index.tvim``
    ...
    def load(
        ...
        if version != _DOCSTORE_SCHEMA_VERSION:
            raise ValueError(
                ...
                f"this turbo_graph expects version {_DOCSTORE_SCHEMA_VERSION}"
            )
