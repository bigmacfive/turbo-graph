"""Agno VectorDb backed by turbo_graph's quantized index.

Install with: ``pip install turbo-graph[agno]``.
...
try:
    ...
    raise ImportError(
        "agno is required to use turbo_graph.agno. "
        "Install with: pip install turbo-graph[agno]"
    ) from exc
...
class TurboQuantVectorDb(VectorDb):
    ...
    def __init__(
        ...
        *,
        ...
        :param distance: Only :class:`Distance.cosine` is supported.
            turbo_graph stores unit-normalized vectors, so the kernel's raw
        ...
        if embedder is None:
            raise ValueError(
                "`embedder` is required; turbo_graph needs the embedder's "
        ...
        if distance != Distance.cosine:
            raise ValueError(
                ...
                f"got {distance}. turbo_graph stores unit-normalized vectors."
            )
    ...
    def get_supported_search_types(self) -> List[SearchType]:
        ...
        # / lexical index that turbo_graph doesn't ship. Return shape
    ...
    def _load_from(self, folder: Path) -> None:
        ...
        if version != _DOCSTORE_SCHEMA_VERSION:
            raise ValueError(
                ...
                f"turbo_graph expects {_DOCSTORE_SCHEMA_VERSION}"
            )
