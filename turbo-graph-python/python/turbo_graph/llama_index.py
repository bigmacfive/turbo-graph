"""LlamaIndex VectorStore backed by turbo_graph's quantized index.

Install with: ``pip install turbo-graph[llama-index]``.
"""
...
try:
    ...
    raise ImportError(
        "llama-index-core is required to use turbo_graph.llama_index. "
        "Install with: pip install turbo-graph[llama-index]"
    ) from exc
...
class TurboQuantVectorStore(BasePydanticVectorStore):
    ...
    def get(self, text_id: str) -> List[float]:
        ...
        embedding for a given node id. turbo_graph discards full-precision
        ...
        raise NotImplementedError(
            ...
            "embedding because turbo_graph quantizes vectors to 2-4 bits per "
    ...
    def get_nodes(
        ...
        here because it doesn't store nodes), turbo_graph keeps node text
    ...
    def _single_filter_match(metadata: dict[str, Any], f: MetadataFilter) -> bool:
        ...
        if op == FilterOperator.TEXT_MATCH:
            ...
            # both sides must be strings. Previous turbo_graph impl
    ...
    def query(self, query: VectorStoreQuery, **_: Any) -> VectorStoreQueryResult:
        ...
        # or sparse-dense fusion). turbo_graph discards full precision after
        ...
        if query.mode != VectorStoreQueryMode.DEFAULT:
            raise NotImplementedError(
                ...
                "full-precision vectors which turbo_graph discards after "
    ...
    def from_persist_path(
        ...
        if version not in _NODES_SCHEMA_COMPAT:
            raise ValueError(
                ...
                f"this turbo_graph accepts versions {list(_NODES_SCHEMA_COMPAT)}"
            )
