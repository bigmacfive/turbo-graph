"""Haystack DocumentStore backed by turbo_graph's quantized index.

Install with: ``pip install turbo-graph[haystack]``.
...
from ._turbo_graph import IdMapIndex
...
try:
    ...
    raise ImportError(
        "haystack-ai is required to use turbo_graph.haystack. "
        "Install with: pip install turbo-graph[haystack]"
    ) from exc


class TurboQuantDocumentStore:
    """Haystack DocumentStore backed by a :class:`~turbo_graph.IdMapIndex`.
    ...
    def __init__(
        ...
        *,
        ...
        :param embedding_similarity_function: ``"cosine"`` (default) or
            ...
            during retrieval. Defaults to ``"cosine"`` because turbo_graph
        ...
        :param return_embedding: Whether retrieval methods should leave
            ...
            turbo_graph never has the full-precision embedding available, so
        ...
        self.executor = async_executor or ThreadPoolExecutor(
            thread_name_prefix=f"async-turbo_graph-docstore-executor-{id(self)}",
            max_workers=1,
        )
    ...
    def embedding_retrieval(
        ...
        ``return_embedding`` set in the constructor. turbo_graph never has
    ...
    def load_from_disk(
        ...
        if version not in cls._DOCSTORE_SCHEMA_COMPAT:
            raise ValueError(
                ...
                f"this turbo_graph accepts versions {list(cls._DOCSTORE_SCHEMA_COMPAT)}"
            )
        store = cls(
    ...
    def _reconstruct(
        ...
        if score is not None and scale_score:
            ...
            # function. turbo_graph uses unit-normalized vectors so the cosine
