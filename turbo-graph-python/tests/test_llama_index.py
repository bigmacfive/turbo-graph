...
try:
    ...
    pytest.skip(
        f"llama-index-core is unavailable or incompatible: {exc}",
...
def test_query_text_match_is_case_sensitive():
    ...
    # SENSITIVE substring check. An earlier turbo_graph impl lowercased both
...
def test_query_returns_node_with_full_field_fidelity():
    ...
    # turbo_graph's write -> query round-trip. The previous narrow side-car
...
def test_persist_v1_schema_still_loads_with_narrow_fidelity(tmp_path):
    # A nodes.json written by an older turbo_graph (schema_version=1, narrow
...
def test_query_rejects_non_default_mode():
    # MMR / SVM / hybrid all need full-precision vectors which turbo_graph
...
def test_query_returned_node_always_has_none_embedding():
    # turbo_graph discards full-precision embeddings after quantization.
