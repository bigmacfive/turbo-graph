...
try:
    ...
    pytest.skip(
        f"agno is unavailable or incompatible: {exc}",
...
try:
    ...
    pytest.skip(
        f"agno reranker API is unavailable or incompatible: {exc}",
        allow_module_level=True,
    )
...
def test_search_does_not_dedupe_distinct_documents_with_identical_content():
    # LanceDb dedupes search results by content string; turbo_graph
...
def test_save_before_create_raises():
    ...
    with pytest.raises(RuntimeError, match="no index to save"):
        db.save("/tmp/nonexistent-turbo_graph-store")
...
def test_search_results_content_origin_divergence_from_lancedb():
    ...
    # JSON of {id, name, meta_data, content_id, usage} only); turbo_graph
    ...
    # change (turbo_graph preserving it where LanceDb doesn't) is a
    # deliberate decision.
    embedder = StubEmbedder(DIM)
    db = TurboQuantVectorDb(embedder=embedder)
    db.create()
...
def test_search_results_embedding_is_none_divergence_from_lancedb():
    ...
    # turbo_graph quantizes vectors to 2-4 bits per dim and discards the
