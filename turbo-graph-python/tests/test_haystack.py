...
try:
    ...
    pytest.skip(
        f"haystack-ai is unavailable or incompatible: {exc}",
        allow_module_level=True,
    )

from turbo_graph.haystack import TurboQuantDocumentStore
...
@component
class SerializableProbeRetriever:
    ...
    def from_dict(cls, data):
        init_parameters = data["init_parameters"].copy()
        init_parameters["document_store"] = TurboQuantDocumentStore.from_dict(
...
# users compare turbo_graph's store against InMemoryDocumentStore". ----
...
def test_return_embedding_flag_is_inert_for_turbo_graph():
...
def test_save_writes_json_sidecar(tmp_path):
    ...
    # turbo_graph-saved store should be able to read it with a text editor.
    import json
...
def test_storage_property_returns_documents_by_id():
    ...
    # Embeddings always None — turbo_graph doesn't keep them.
...
def test_pipeline_to_dict_from_dict_roundtrip():
    ...
    pipeline.add_component(
        "retriever", SerializableProbeRetriever(document_store=store)
    )
    ...
    assert isinstance(restored_retriever.document_store, TurboQuantDocumentStore)
    assert restored_retriever.document_store._index.dim == DIM
    assert restored_retriever.document_store._bit_width == 4
...
def test_to_dict_includes_all_init_params_and_type_key():
    ...
    assert serialized["type"] == "turbo_graph.haystack.TurboQuantDocumentStore"
