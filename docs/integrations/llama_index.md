...
from turbo_graph.llama_index import TurboQuantVectorStore
...
`query_embedding` is **required**. turbo_graph doesn't embed query text itself; the calling component (retriever / query engine) is responsible for that.
...
Supported operators on `MetadataFilter`: `EQ`, `NE`, `GT`, `LT`, `GTE`, `LTE`, `IN`, `NIN`, `TEXT_MATCH`, `TEXT_MATCH_INSENSITIVE`, `CONTAINS`, `IS_EMPTY`, `ANY`, and `ALL`. Conditions: `AND`, `OR`, and `NOT`. Nested `MetadataFilters` work.

Filter semantics match `SimpleVectorStore`'s reference implementation — notably, every operator except `IS_EMPTY` returns `False` when the filter key is missing from the document's metadata. `TEXT_MATCH` is case-sensitive; use `TEXT_MATCH_INSENSITIVE` when explicit case-folding is desired.
...
- **MMR is not supported.** Max-marginal-relevance retrieval requires the full-precision embedding of each candidate to compute pairwise diversity; turbo_graph discards full-precision vectors after quantization.
