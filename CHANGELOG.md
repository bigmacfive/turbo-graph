...
All notable changes to turbo_graph are recorded here. The format is based on
[Keep a Changelog](https://keepachangelog.com/en/1.1.0/) and the project follows
[Semantic Versioning](https://semver.org/spec/v2.0.0.html).

The Rust crate (`turbo-graph` on crates.io) and the Python distribution
(`turbo-graph` on PyPI) ship together from the public 0.1 line onward.
Older entries are retained as pre-0.1 development history.
...
- **Python `GraphMemoryIndex` core operating API** for constrained RAG:
  `add_records`, `add_node`, graph links, metadata-aware graph search,
  batch search, optional upstream `candidate_ids`, explain reports, cache
  stats, cache budget/trim/clear controls, and graph/index persistence.
- **Typed Python package surface** with `py.typed`, API stubs,
  `__version__`, and graph inspection helpers for `record(id)` and
  `neighbors(id)`.
- **Runnable Python graph-memory RAG example** covering candidate ids,
  batch search, explain telemetry, cache controls, and persistence.
- **Production graph-memory layer on top of the turbovec-compatible
  TurboQuant core**: weighted graph expansion, tag/source/time indexes,
  cached slot-mask compilation, graph-aware rerank, and telemetry.
- **Faster graph+metadata mask composition**: `SlotMask` now supports batched
  union/intersection over packed words and recomputes active blocks with
  per-block popcount, cutting repeated rebuild work for multi-tag/source/time
  RAG filters.
- **Release hardening**: unified Rust/Python version `0.1.0`, public
  repository metadata for `bigmacfive/turbo-graph`, security policy,
  package hygiene, format checks, clippy checks, and wheel smoke tests.

### Positioning

- turbo-graph keeps the proven TurboQuant lineage from
  [RyanCodrai/turbovec](https://github.com/RyanCodrai/turbovec), then
  targets the next bottleneck: repeated construction of
  `graph ∩ tag ∩ source ∩ time ∩ candidates` views in real RAG systems.
- Use turbovec for light filters and global top-k. Use turbo-graph when
  constraints, cache reuse, explainability, and graph-aware rerank are
  part of the product.

<details>
<summary>Pre-0.1 development history</summary>

## turbo_graph 0.7.0 (Python package) + turbo_graph 0.8.0 (Rust crate) — 2026-05-30
...
### turbo_graph — Rust crate (current: 0.7.0 → next: 0.8.0)
...
### turbo_graph — Python package (current: 0.6.0 → next: 0.7.0)
...
- **`turbo_graph.langchain.TurboQuantVectorStore`**: `similarity_search`,
...
- **`turbo_graph.haystack.TurboQuantDocumentStore`**: `Document.blob` and
...
- **`turbo_graph.llama_index.TurboQuantVectorStore`**: full `BaseNode`
  ...
  - `add()` rejects intra-batch duplicate `node_id`s with a clear
    ...
    first node's vector).
- **`turbo_graph.agno.TurboQuantVectorDb`**: `embedder` is now threaded
...
## turbo_graph 0.6.0 (Python package) + turbo_graph 0.7.0 (Rust crate) — 2026-05-27

### turbo_graph — Rust crate (current: 0.6.0 → next: 0.7.0)
...
### turbo_graph — Python package (current: 0.5.3 → next: 0.6.0)
...
## turbo_graph 0.5.3 (Python package) + turbo_graph 0.6.0 (Rust crate) — 2026-05-25

### turbo_graph — Rust crate (current: 0.5.0 → next: 0.6.0)
...
- **BREAKING:** `TurboQuantIndex::new`, `TurboQuantIndex::new_lazy`,
  ...
  input. The new `turbo_graph::ConstructError` enum covers `bit_width`
  ...
  existing constructor calls. Mirrors the `AddError` pattern from the
  previous release.
...
- **Linux wheels now actually import.** Every Linux wheel since
  ...
  deps) into `turbo_graph.libs/`. Linux wheel size grows from ~1.8 MB
...
## turbo_graph 0.5.2 (Python package) + turbo_graph 0.5.0 (Rust crate) — 2026-05-21

### turbo_graph — Rust crate (current: 0.4.1 → next: 0.5.0)

#### Changed

- **BREAKING:** `TurboQuantIndex::add_2d`, `IdMapIndex::add_with_ids_2d`,
  and `IdMapIndex::add_with_ids` now return `Result<(), AddError>`
  instead of panicking on invalid input. The new `turbo_graph::AddError`
...
### turbo_graph — Python package (current: 0.5.1 → next: 0.5.2)
...
## turbo_graph 0.5.1 (Python package) + turbo_graph 0.4.1 (Rust crate) — 2026-05-18

### turbo_graph — Rust crate (current: 0.4.0 → next: 0.4.1)

#### Added

- **Block-level early exit for selective mask searches** (closes
  [#30](https://github.com/bigmacfive/turbo-graph/issues/30)). When a
...
- **`turbo_graph::search::BLOCKS_SKIPPED_BY_MASK`** — atomic counter
...
### turbo_graph — Python package (current: 0.5.0 → next: 0.5.1)
...
- **Block-level early exit for selective `search_with_mask` calls.**
  ...
  [#30](https://github.com/bigmacfive/turbo-graph/issues/30).

## turbo_graph 0.5.0 (Python package) + turbo_graph 0.4.0 (Rust crate) — 2026-05-18

> **BREAKING** — on-disk file format version bumped from 1 to 2.
> Existing `.tv` and `.tvim` files written by turbo_graph ≤ 0.4.3 cannot
...
```
this .tv file was written by turbo_graph ≤ 0.4.3 (format version 1).
It is incompatible with turbo_graph 0.4.4+ because the per-vector scalar's
meaning changed. Rebuild this index from the source vectors using
turbo_graph 0.4.4 or later.
```

### turbo_graph — Rust crate (current: 0.3.0 → next: 0.4.0)
...
### turbo_graph — Python package (current: 0.4.3 → next: 0.5.0)
...
- **On-disk format version bumped to 2** for both `.tv` and `.tvim`.
  ...
  bumped from 1 to 2.
- **Loading a turbo_graph ≤ 0.4.3 index raises with a clear error.**
...
- **`turbo_graph.haystack.TurboQuantDocumentStore` clamps cosine scores
...
## turbo_graph 0.4.3 (Python package) — 2026-05-18

### turbo_graph — Python package (current: 0.4.2 → next: 0.4.3)

#### Added

- **Windows x64 wheel** (closes [#31](https://github.com/bigmacfive/turbo-graph/issues/31)).
  Prior releases shipped only Linux x86_64/aarch64, macOS aarch64, and an
  sdist — Windows users running `pip install turbo_graph` fell through to
  ...
  in [#33](https://github.com/bigmacfive/turbo-graph/pull/33).
  ...
  tracked separately in [#34](https://github.com/bigmacfive/turbo-graph/issues/34).
...
## turbo_graph 0.4.2 (Python package) — 2026-05-17

### turbo_graph — Python package (current: 0.4.1 → next: 0.4.2)
...
- **`numpy` is now a declared runtime dependency.** The Python package
  ...
  This broke `pip install turbo_graph[agno]` in clean environments because
...
## turbo_graph 0.4.1 (Python package) — 2026-05-17

### turbo_graph — Python package (current: 0.4.0 → next: 0.4.1)

#### Added

- **Agno integration** (`turbo_graph.agno`). New `TurboQuantVectorDb` class
  ...
  - Constructor restricts `search_type=vector` and `distance=cosine`
    — turbo_graph doesn't ship a BM25/lexical index and stores
  ...
  - Install: `pip install turbo_graph[agno]`.

## turbo_graph 0.3.0 (Rust crate) — 2026-05-17

## turbo_graph 0.3.0 (Rust crate) — 2026-05-17

### turbo_graph — Rust crate (current: 0.2.0 → next: 0.3.0)
...
- **Search-time filtering.** New methods restrict the returned top-k to
  ...
  ([#21](https://github.com/bigmacfive/turbo-graph/issues/21))
...
## turbo_graph 0.4.0 (Python package) — 2026-05-17

### turbo_graph — Python package (current: 0.3.0 → next: 0.4.0)
...
- **Haystack integration** (`turbo_graph.haystack`):
  ...
  - Constructor accepts `embedding_similarity_function`
    (`"cosine"` default, since turbo_graph stores unit-normalized
...
- **LangChain integration** (`turbo_graph.langchain`):
  ...
  - `max_marginal_relevance_search` / `_by_vector` /
    ...
    candidate vectors that turbo_graph discards after encoding.
  - Persistence methods renamed: `save_local → dump`, `load_local →
    load`, matching the reference.

- **LlamaIndex integration** (`turbo_graph.llama_index`):
  `TurboQuantVectorStore` is now a structural drop-in for
...
[Unreleased]: https://github.com/bigmacfive/turbo-graph/compare/py-v0.4.2...HEAD
[py-v0.4.2]: https://github.com/bigmacfive/turbo-graph/compare/py-v0.4.1...py-v0.4.2
[py-v0.4.1]: https://github.com/bigmacfive/turbo-graph/compare/py-v0.4.0...py-v0.4.1
